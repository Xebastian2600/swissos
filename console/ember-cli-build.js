...
module.exports = function (defaults) {
    const app = new EmberApp(defaults, {
        ...
        fingerprint: {
            exclude: ['leaflet/', 'leaflet-images/', 'socketcluster-client.min.js', 'swissos.config.json', 'extensions.json'],
        },
    ...
    if (toBoolean(process.env.DISABLE_RUNTIME_CONFIG)) {
        runtimeConfigTree = writeFile('swissos.config.json', '{}');
    } else {
        runtimeConfigTree = new Funnel('.', {
            files: ['swissos.config.json'],
            destDir: '/',
        });
    }
