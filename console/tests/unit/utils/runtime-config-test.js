...
const CACHE_KEY = 'swissos_runtime_config';
const CACHE_VERSION_KEY = 'swissos_runtime_config_version';
...
module('Unit | Utility | runtime-config', function (hooks) {
    ...
    test('clearRuntimeConfigCache removes both cache entries', function (assert) {
        window.localStorage.setItem('swissos_runtime_config', JSON.stringify({ config: {}, timestamp: Date.now() }));
        window.localStorage.setItem('swissos_runtime_config_version', '1.0.0');

        clearRuntimeConfigCache();

        assert.strictEqual(window.localStorage.getItem('swissos_runtime_config'), null);
        assert.strictEqual(window.localStorage.getItem('swissos_runtime_config_version'), null);
    });
    ...
    test('loadRuntimeConfig falls through to the server when the cache is empty', async function (assert) {
        ...
        assert.strictEqual(this.requests[0][0], '/swissos.config.json');
