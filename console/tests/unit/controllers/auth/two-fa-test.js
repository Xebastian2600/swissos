...
module('Unit | Controller | auth/two-fa | verification flow', function (hooks) {
    ...
    test('verifyCode posts the code, authenticates and lands in the console', async function (assert) {
        ...
        controller.identity = 'karl@hostoscollective.com';
        controller.verificationCode = '123456';

        await controller.verifyCode();

        assert.deepEqual(this.posted, [{ path: 'two-fa/verify', payload: { token: 'tok', code: '123456', clientToken: 'client-tok', identity: 'karl@hostoscollective.com' } }]);
        assert.deepEqual(this.owner.lookup('service:session').authenticated, [
            { authenticator: 'authenticator:fleetbase', credentials: { authToken: 'auth-token', identity: 'karl@hostoscollective.com' } },
        ]);
    ...
    test('resendCode restarts the countdown with the new client token', async function (assert) {
        ...
        assert.deepEqual(this.posted, [{ path: 'two-fa/resend', payload: { identity: 'karl@hostoscollective.com', token: 'tok' } }]);
    ...
    test('cancelTwoFactor invalidates the session and returns to login', async function (assert) {
        ...
        assert.deepEqual(this.posted, [{ path: 'two-fa/invalidate', payload: { identity: 'karl@hostoscollective.com', token: 'tok' } }]);
