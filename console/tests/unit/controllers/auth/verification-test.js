...
module('Unit | Controller | auth/verification | resend and verify', function (hooks) {
    ...
    test('verifyCode authenticates and lands in the console when a token comes back', async function (assert) {
        ...
        controller.email = 'karl@hostoscollective.com';

        await controller.verifyCode.perform();

        assert.deepEqual(this.posted, [{ path: 'auth/verify-email', payload: { token: 'tok', code: '123456', email: 'karl@hostoscollective.com', authenticate: true } }]);
        ...
        assert.deepEqual(this.notifications().infos, ['Welcome to SwissOS']);
    });
    ...
    test('resendEmail opens the email modal and sends the code with the session', async function (assert) {
        ...
        const modal = this.modal({ email: 'karl@hostoscollective.com' });
        await options.confirm(modal);

        assert.deepEqual(this.posted.at(-1), { path: 'onboard/send-verification-email', payload: { email: 'karl@hostoscollective.com', session: 'session-token' } });
        assert.deepEqual(modal.calls, ['startLoading', 'done']);
