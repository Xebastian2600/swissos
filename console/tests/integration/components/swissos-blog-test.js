...
import SwissosBlogComponent from '@fleetbase/console/components/swissos-blog';
import { captureComponent } from '@fleetbase/console/tests/helpers/capture-component';

module('Integration | Component | swissos-blog', function (hooks) {
    ...
    test('it renders blog posts from the lookup endpoint', async function (assert) {
        ...
        await render(hbs`<SwissosBlog />`);

        assert.deepEqual(fetch.requests, ['lookup/fleetbase-blog']);
        assert.dom('.swissos-blog').containsText('SwissOS Blog');
        ...
        assert.dom('.swissos-blog').containsText('May 6, 2026');
    });

    test('it renders an empty state when no posts are available', async function (assert) {
        await render(hbs`<SwissosBlog />`);

        assert.dom('.swissos-blog').containsText('No blog posts are available right now.');
    });
});
 ...
 */
module('Integration | Component | swissos-blog | loading', function (hooks) {
    setupRenderingTest(hooks);
    ...
    hooks.beforeEach(function () {
        ...
        const captured = captureComponent(this.owner, 'swissos-blog', SwissosBlogComponent);
        this.build = async () => {
            await render(hbs`<SwissosBlog />`);
    ...
    test('an empty cache loads the posts and caches them', async function (assert) {
        ...
        assert.deepEqual(component.localCache.get('swissos-blog-data'), this.getResponse, 'the posts are cached');
        assert.ok(component.localCache.get('swissos-blog-data-expiration'), 'with an expiry stamp');
    });
 ...
 */
module('Integration | Component | swissos-blog | cache', function (hooks) {
    setupRenderingTest(hooks);
    ...
    test('a second mount is served entirely from the cache', async function (assert) {
        ...
        assert.deepEqual(second.posts, first.localCache.get('swissos-blog-data'), 'it renders what was cached');
        assert.strictEqual(second.posts[0].title, 'Cached Post');
    });

    test('an expired cache is refetched', async function (assert) {
        const first = await this.build();
        first.localCache.set('swissos-blog-data-expiration', new Date('2020-01-01T00:00:00Z'));
    ...
    test('a cache holding something other than a list is refetched', async function (assert) {
        const first = await this.build();
        first.localCache.set('swissos-blog-data', { error: 'corrupted' });
    ...
    test('an empty response is not cached', async function (assert) {
        ...
        assert.strictEqual(component.localCache.get('swissos-blog-data'), undefined, 'and nothing is written to the cache');
    });
    ...
    test('a response arriving after teardown is dropped', async function (assert) {
        ...
        assert.strictEqual(component.localCache.get('swissos-blog-data'), undefined, 'and nothing is cached');
    });
});
