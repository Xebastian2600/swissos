...
module('Integration | Component | github-card', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        window.localStorage.setItem(
            ...
            JSON.stringify({
                'swissos-github-data': {
                    full_name: 'karlrodriguez/swissos',
                    html_url: 'https://github.com/karlrodriguez/swissos',
                    owner: { avatar_url: '/images/swissos-logo-svg.svg' },
                },
                'swissos-github-data-expiration': notExpired,
                'swissos-github-tags': [{ name: 'v0.7.53' }],
                'swissos-github-tags-expiration': notExpired,
            })
        );
    });
    ...
    test('it renders the repository card from cached data', async function (assert) {
        ...
        assert.dom('.swissos-github-card').exists('the card renders');
        assert.dom(this.element).containsText('karlrodriguez/swissos');
    });
...
module('Integration | Component | github-card | fetching', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        this.responses = {
            'https://api.github.com/repos/karlrodriguez/swissos': { ok: true, body: { full_name: 'karlrodriguez/swissos' } },
            'https://api.github.com/repos/karlrodriguez/swissos/tags': { ok: true, body: [{ name: 'v9.9.9' }] },
        };
    ...
    test('an empty cache fetches the repository and its tags, then caches both', async function (assert) {
        ...
        assert.deepEqual(this.requested.slice().sort(), ['https://api.github.com/repos/karlrodriguez/swissos', 'https://api.github.com/repos/karlrodriguez/swissos/tags']);
        assert.dom(this.element).containsText('karlrodriguez/swissos');
        assert.dom(this.element).containsText('v9.9.9');

        const cache = JSON.parse(window.localStorage.getItem(LOCAL_CACHE_KEY));
        assert.deepEqual(cache['swissos-github-data'], { full_name: 'karlrodriguez/swissos' }, 'the repository is cached');
        assert.deepEqual(cache['swissos-github-tags'], [{ name: 'v9.9.9' }], 'the tags are cached');
        assert.ok(cache['swissos-github-data-expiration'], 'with an expiry stamp');
        assert.ok(cache['swissos-github-tags-expiration']);
    });
    ...
    test('an expired cache is refetched rather than reused', async function (assert) {
        ...
        window.localStorage.setItem(
            LOCAL_CACHE_KEY,
            JSON.stringify({
                'swissos-github-data': { full_name: 'stale/repo' },
                'swissos-github-data-expiration': expired,
                'swissos-github-tags': [{ name: 'v0.0.0-stale' }],
                'swissos-github-tags-expiration': expired,
            })
        );
        _resetStorages();
    ...
    test('a failed response leaves the built-in defaults in place', async function (assert) {
        ...
        assert.dom('.swissos-github-card').exists('the card still renders');
    ...
    test('the release link points at the tag when one is known', async function (assert) {
        ...
        assert.strictEqual(component.releaseUrl, 'https://github.com/karlrodriguez/swissos/releases/tag/v9.9.9');

        component.tags = [];
        assert.deepEqual(component.latestRelease, { name: 'v0.0.1' }, 'with no tags a placeholder release is used');
        assert.strictEqual(component.releaseUrl, 'https://github.com/karlrodriguez/swissos/releases/tag/v0.0.1');
    });
    ...
    test('the release link drops the tag segment when the release has no name', async function (assert) {
        ...
        assert.strictEqual(component.releaseUrl, 'https://github.com/karlrodriguez/swissos/releases', 'a blank tag name leaves the plain releases URL');
    });
});
