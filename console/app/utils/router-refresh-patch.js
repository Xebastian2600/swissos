...
/**
 * SwissOS Router Refresh Bug Fix Utility
 *
 ...
 *
 * @author Karl Rodriguez <karl@hostoscollective.com>
 * @version 1.0.0
 */
...
export function patchRouterRefresh(application) {
    if (!application || typeof application.lookup !== 'function') {
        debug('[SwissOS Router Patch] Invalid application instance provided');
        return;
    }
    ...
    try {
        ...
        if (!router || !router._routerMicrolib) {
            debug('[SwissOS Router Patch] Router not found or invalid');
            return;
        }

        // Check if already patched
        if (router._routerMicrolib._swissosRefreshPatched) {
            debug('[SwissOS Router Patch] Already applied, skipping');
            return;
        }
        ...
        // Mark as patched
        router._routerMicrolib._swissosRefreshPatched = true;

        debug('[SwissOS Router Patch] Successfully applied router refresh bug fix');
    } catch (error) {
        debug('[SwissOS Router Patch] Failed to apply patch: ' + error.message);
    }
}
...
export function suppressRouterRefreshErrors(application) {
    if (!application) {
        debug('[SwissOS Router Patch] Invalid application instance for error suppression');
        return;
    }
    ...
    try {
        ...
        window.addEventListener('unhandledrejection', (event) => {
            ...
            if (typeof error?.message === 'string' && error?.message.includes("You didn't provide enough string/numeric parameters to satisfy all of the dynamic segments")) {
                debug('[SwissOS Router Patch] Suppressed known Ember route refresh bug: ' + error.message);
        ...
        debug('[SwissOS Router Patch] Error suppression handlers installed');
    } catch (error) {
        debug('[SwissOS Router Patch] Failed to install error suppression: ' + error.message);
    }
}
...
export default function applyRouterFix(application, options = {}) {
    ...
    debug('[SwissOS Router Patch] Applying Ember router refresh bug fix...');
    ...
    debug('[SwissOS Router Patch] Router fix application complete');
}
