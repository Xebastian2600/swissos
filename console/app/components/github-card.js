...
export default class GithubCardComponent extends Component {
    ...
    @computed('data.releases_url', 'latestRelease.name') get releaseUrl() {
        let url = 'https://github.com/karlrodriguez/swissos/releases';
    ...
    @task *getRepositoryData() {
        ...
        const cachedData = this.localCache.get('swissos-github-data');
        const expiration = this.localCache.get('swissos-github-data-expiration');
        ...
        if (cachedData && expiration && !isPast(new Date(expiration))) {
            ...
            // Fetch new data
            const response = yield fetch('https://api.github.com/repos/karlrodriguez/swissos', { cache: 'default' });
            ...
            if (response.ok) {
                ...
                this.localCache.set('swissos-github-data', this.data);
                this.localCache.set('swissos-github-data-expiration', add(new Date(), { hours: 6 }));
            }
        }
    }
    ...
    @task *getRepositoryTags() {
        ...
        const cachedTags = this.localCache.get('swissos-github-tags');
        const expiration = this.localCache.get('swissos-github-tags-expiration');
        ...
        if (cachedTags && expiration && !isPast(new Date(expiration))) {
            ...
            // Fetch new tags
            const response = yield fetch('https://api.github.com/repos/karlrodriguez/swissos/tags', { cache: 'default' });
            ...
            if (response.ok) {
                ...
                this.localCache.set('swissos-github-tags', this.tags);
                this.localCache.set('swissos-github-tags-expiration', add(new Date(), { hours: 6 }));
            }
        }
    }
}
