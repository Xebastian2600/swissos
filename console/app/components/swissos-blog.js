import Component from '@glimmer/component';
import { tracked } from '@glimmer/tracking';
import { inject as service } from '@ember/service';
import { isArray } from '@ember/array';
import { debug } from '@ember/debug';
import { storageFor } from 'ember-local-storage';
import { add, format, isPast, isValid } from 'date-fns';
import { task } from 'ember-concurrency';

export default class SwissosBlogComponent extends Component {
    @storageFor('local-cache') localCache;
    @service fetch;
    @tracked posts = [];

    constructor() {
        super(...arguments);
        this.loadBlogPosts.perform();
    }

    get formattedPosts() {
        return this.posts.map((post) => {
            return {
                ...post,
                formattedDate: this.formatPublishedDate(post.pubDate),
            };
        });
    }

    formatPublishedDate(value) {
        if (!value) {
            return '';
        }

        const date = new Date(value);

        if (!isValid(date)) {
            return value;
        }

        return format(date, 'MMM d, yyyy');
    }

    @task *loadBlogPosts() {
        // Check if cached data and expiration are available
        const cachedData = this.localCache.get('swissos-blog-data');
        const expiration = this.localCache.get('swissos-blog-data-expiration');

        // Check if the cached data is still valid
        if (cachedData && isArray(cachedData) && expiration && !isPast(new Date(expiration))) {
            // Use cached data
            this.posts = cachedData;
        } else {
            // Fetch new data
            try {
                // 'lookup/fleetbase-blog' is an upstream endpoint name served by the vendored
                // core-api package — it is not a SwissOS-owned route, so it keeps its name.
                const data = yield this.fetch.get('lookup/fleetbase-blog');

                // The component can be torn down while this request is in flight; writing
                // tracked state afterwards throws "Cannot create a new tag ... after it has
                // been destroyed".
                if (this.isDestroying || this.isDestroyed) {
                    return;
                }

                this.posts = isArray(data) ? data : [];
                if (data) {
                    this.localCache.set('swissos-blog-data', data);
                    this.localCache.set('swissos-blog-data-expiration', add(new Date(), { hours: 6 }));
                }
            } catch (err) {
                debug('Failed to load blog: ' + err.message);
            }
        }
    }
}
