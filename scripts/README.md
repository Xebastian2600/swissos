...
- `console/.npmrc`
- `api/composer.json`
- `.swissos-dev-links.json`

The `.swissos-dev-links.json` file stores original dependency ranges so links can be reversed. It is ignored by git.
...
`docker-install.sh` is the interactive SwissOS Docker setup wizard. It checks required local tools, asks for core environment settings, generates local configuration, and guides Docker Compose setup.
...
The script expects Docker, Docker Compose v2, git, and OpenSSL to be available. It warns when common SwissOS ports are already in use but does not treat that as a hard failure.
