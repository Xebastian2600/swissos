...
# SwissOS Docker installer — interactive setup wizard
# -------------------------------------------------------
# Usage:
...
echo
echo -e "${BOLD}🚀  SwissOS Installation Wizard${RESET}"
echo
...
if $NON_INTERACTIVE; then
  ...
  read -rp "Application name [SwissOS]: " APP_NAME_INPUT
  APP_NAME="${APP_NAME_INPUT:-SwissOS}"
fi
...
MAIL_FROM_ADDRESS="karl@hostoscollective.com"; MAIL_FROM_NAME="$APP_NAME"
...
cat > "${CONFIG_DIR}/swissos.config.json.tmp" <<JSON
{
...
JSON
mv -f "${CONFIG_DIR}/swissos.config.json.tmp" "${CONFIG_DIR}/swissos.config.json"
...
###############################################################################
section "Starting SwissOS Containers"
...
echo -e "  ${BOLD}🏁  SwissOS Installation Complete${RESET}"
printf '%0.s═' {1..60}; echo
echo
echo "  📍  Endpoints"
printf "      API     → %s://%s:8000\n"    "$SCHEME_API"     "$HOST"
